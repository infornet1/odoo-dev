...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        ...
        for slip in self:
            ...
            # Create enhanced reference string
            reference_parts = [slip.number]

            # Check if this is a liquidation payslip
            if slip.struct_id and slip.struct_id.code == 'LIQUID_VE':
                reference_parts.append('Liquidación')

            # Add employee name
            if slip.employee_id:
                reference_parts.append(slip.employee_id.name)

            # Add date range
            if slip.date_from and slip.date_to:
                date_from_str = slip.date_from.strftime('%d/%m/%Y')
                date_to_str = slip.date_to.strftime('%d/%m/%Y')
                reference_parts.append(f"{date_from_str}-{date_to_str}")

            enhanced_reference = ' '.join(reference_parts)

            move_dict = {
                'narration': name,
                'ref': enhanced_reference,
            ...
            if slip.company_id.currency_id.compare_amounts(
                ...
                adjust_credit = (0, 0, {
                    ...
                    'partner_id': partner_id,  # Use employee's partner
                    'account_id': acc_id,
                    'journal_id': slip.journal_id.id,
                    'date': slip.date or slip.date_to,
                    'debit': 0.0,
