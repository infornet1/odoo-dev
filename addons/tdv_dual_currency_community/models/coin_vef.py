...
class HrPayslip(models.Model):
    ...
    @api.depends('conversion_currency_id', 'line_ids.total')
    def _compute_wage_in_ves(self):
        for payslip in self:
            ...
            # Calculate total from payslip lines
            total_amount = sum(line.total for line in payslip.line_ids)

            if payslip.company_id.currency_id and conversion_currency:
                payslip.wage_in_ves = payslip.company_id.currency_id._convert(
                    total_amount,
                    conversion_currency,
