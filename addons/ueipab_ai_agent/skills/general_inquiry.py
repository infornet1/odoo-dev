...
_INSTITUTIONAL_KNOWLEDGE = (
    ...
    "\n"
    "**Cómo se registra la asistencia (según tipo de empleado):**\n"
    "\n"
    "- *Personal docente (profesores):* La asistencia se registra automáticamente en Odoo cada vez que "
    "el docente ingresa y registra la asistencia estudiantil en el sistema de Control de Asistencias "
    "(la aplicación web que usan para pasar lista a los alumnos). No necesitan hacer nada adicional — "
    "el sistema sincroniza automáticamente todos los días a las 6:30 PM. Horario registrado: 7:00 AM a 1:30 PM.\n"
    "- *Personal administrativo y de mantenimiento:* La asistencia se registra por los medios habituales "
    "definidos por RRHH.\n"
    "\n"
    ...
    "- *Soy docente y el reporte me marca ausente pero pasé lista ese día* → Si registraste la asistencia estudiantil en el sistema de Control de Asistencias, tu asistencia debería aparecer. Si hay un error, usa el enlace \"Solicitar Corrección de Asistencia\" en el correo del reporte.\n"
    "- *Soy docente y no pasé lista en el sistema de Control de Asistencias ese día* → Si no registraste en el sistema, no hay evidencia automática de tu presencia. Si estuviste presente por otro motivo, usa el enlace de corrección e indica el motivo (ej: reunión, sustitución, etc.).\n"
    "- *Tengo un error en mi registro (ausencia que no fue real, falta registro de entrada/salida)* → Pueden solicitar la corrección directamente desde el enlace **\"Solicitar Corrección de Asistencia\"** que aparece en el mismo correo del reporte. Sin necesidad de llamar a RRHH.\n"
    "- *El sistema me marca ausente pero estuve presente* → Usen el enlace \"Solicitar Corrección de Asistencia\" en el correo recibido. Seleccionen la fecha, indiquen la hora de llegada y salida, el motivo (ej: corte de luz) y envíen. RRHH lo revisará y aprobará rápidamente.\n"
    "- *No tengo el correo o perdí el enlace de corrección* → Escriban a recursoshumanos@ueipab.edu.ve indicando nombre, fecha y motivo.\n"
    "- *¿Hay un portal de autogestión?* → El sistema de corrección ya está activo. Cada reporte que llega al correo tiene un botón directo para solicitar correcciones.\n"
    "- *¿Cuánto tiempo tarda en procesarse la corrección?* → RRHH la revisa y aprueba normalmente el mismo día. Recibirán un correo de confirmación.\n"
    "\n"
    "**Cómo responde Glenda:** Si un docente pregunta por qué tiene ausencias, primero pregunta si registró "
    "la asistencia estudiantil en el sistema de Control de Asistencias ese día. Si sí lo hizo, el sistema "
    "debería haberlo registrado automáticamente — sugiere revisar el reporte o contactar a recursoshumanos@ueipab.edu.ve. "
    "Si no lo registró en el sistema, explica que la asistencia automática depende de ese registro y orienta "
    "al enlace de corrección. No compartas datos de asistencia de otros empleados.\n"
