...
class HrPayslip(models.Model):
    ...
    exchange_rate_used = fields.Float(
        ...
        digits=(12, 6),
        help='Exchange rate captured when payslip was computed (USD->VES direct rate)'
    )
    exchange_rate_date = fields.Datetime(
        string='Rate Capture Date',
        help='When this exchange rate was captured',
        readonly=True
    )
    ...
    def action_compute_sheet(self):
        ...
        for payslip in self:
            ...
            # Auto-capture exchange rate if not already set
            if not payslip.exchange_rate_used:
                try:
                    ...
                    _logger.warning(f"Could not capture exchange rate for payslip {payslip.number}: {e}")
