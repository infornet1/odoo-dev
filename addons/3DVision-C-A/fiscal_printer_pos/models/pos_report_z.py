...
class PosReportZ(models.Model):
    ...
    #número de reporte
    number = fields.Char("Número de reporte", required=True)
    ...
    x_fiscal_printer_code = fields.Char("Código de la impresora fiscal", related="x_fiscal_printer_id.serial", store=True)
    ...
    total_exempt = fields.Float("Total exento")

    #total base imponible iva 16
    total_base_iva_16 = fields.Float("Total base imponible")

    #total iva 16
    total_iva_16 = fields.Float("Total iva")

    # total acumulado exento nota de crédito
    total_exempt_nc = fields.Float("Total exento NC")

    # total base imponible iva 16 nota de crédito
    total_base_iva_16_nc = fields.Float("Total base imponible 16 NC")

    # total iva 16 nota de crédito
    total_iva_16_nc = fields.Float("Total iva NC")
    ...
    total_exempt_pos = fields.Float("Total Excento POS", compute="_compute_total_pos")

    #total base imponible iva 16 ventas pos
    total_base_iva_16_pos = fields.Float("Total Base Imponible POS", compute="_compute_total_pos")

    #total iva 16 ventas pos
    total_iva_16_pos = fields.Float("Total IVA POS", compute="_compute_total_pos")

    #total excento ventas pos nota de crédito
    total_exempt_pos_nc = fields.Float("Total Excento POS NC", compute="_compute_total_pos")

    #total base imponible iva 16 ventas pos nota de crédito
    total_base_iva_16_pos_nc = fields.Float("Total Base Imponible POS NC", compute="_compute_total_pos")

    #total iva 16 ventas pos nota de crédito
    total_iva_16_pos_nc = fields.Float("Total IVA POS NC", compute="_compute_total_pos")

    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)

    fac_desde = fields.Char("Factura desde")
    fac_hasta = fields.Char("Factura hasta")
