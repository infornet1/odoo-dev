...
Spreadsheet columns:
  K: SALARIO MENSUAL MAS BONO (semi-monthly)
  L: OTROS BONOS (semi-monthly)
  M: CESTA TICKET MENSUAL PTR (semi-monthly)
  Total VEB = K + L + M
  Total USD = (K + L + M) / 234.8715
"""
...
print(f"\n[2/4] Reading spreadsheet data...")
try:
    # Read data range: D5:M48 (employee name + wage columns)
    # D: Name, K: Salary+Bonus, L: Other Bonuses, M: Cesta Ticket
    data_range = 'D5:M48'
    employee_data = worksheet.get(data_range)
    ...
    for row in employee_data:
        ...
        name = row[0].strip().upper()  # Normalize to uppercase

        # Extract wage components (K, L, M are columns 7, 8, 9 relative to column D which is 0)
        # D=0, E=1, F=2, G=3, H=4, I=5, J=6, K=7, L=8, M=9
        try:
            salary_bonus_veb = float(row[7].replace(',', '').strip()) if len(row) > 7 and row[7] else 0.0
            other_bonus_veb = float(row[8].replace(',', '').strip()) if len(row) > 8 and row[8] else 0.0
            cesta_ticket_veb = float(row[9].replace(',', '').strip()) if len(row) > 9 and row[9] else 0.0

            total_veb = salary_bonus_veb + other_bonus_veb + cesta_ticket_veb
            total_usd = total_veb / VEB_USD_RATE

            spreadsheet_data[name] = {
                'salary_bonus_veb': salary_bonus_veb,
                'other_bonus_veb': other_bonus_veb,
                'cesta_ticket_veb': cesta_ticket_veb,
                'total_veb': total_veb,
                'total_usd': total_usd
            }
        except (ValueError, IndexError) as e:
            print(f"⚠ Warning: Could not parse wage for {name}: {e}")
            continue
...
try:
    # Get all active contracts (env is already available in Odoo shell)
    ...
    for contract in contracts:
        emp_name = contract.employee_id.name.upper()  # Normalize to uppercase
        odoo_data[emp_name] = {
            'wage': contract.wage,
...
for emp_name, sheet_data in sorted(spreadsheet_data.items()):
    if emp_name in odoo_data:
        sheet_wage_usd = sheet_data['total_usd']
...
# Print matches (condensed)
if matches:
    print(f"\n✓ MATCHES ({len(matches)} employees):")
    for match in matches[:5]:  # Show first 5
        print(f"  {match['name']:<28} ${match['sheet_usd']:>12.2f}  ${match['odoo_usd']:>12.2f}  ${match['diff']:>12.4f}")
    if len(matches) > 5:
        print(f"  ... and {len(matches) - 5} more matches")

# Print mismatches (full details)
if mismatches:
    print(f"\n✗ MISMATCHES ({len(mismatches)} employees):")
    for mismatch in mismatches:
        print(f"  {mismatch['name']:<28} ${mismatch['sheet_usd']:>12.2f}  ${mismatch['odoo_usd']:>12.2f}  ${mismatch['diff']:>12.2f} ({mismatch['diff_pct']:>5.2f}%)")
...
if len(matches) == len(spreadsheet_data) and len(mismatches) == 0 and len(spreadsheet_data) > 0:
    ...
    print("   → V2 Revision Plan can be updated with this validation result")
    exit(0)
else:
