...
## 10. Convenio Due-Date Scheduling — 1er Llamado only (UNDER ANALYSIS, 2026-06-11)

**Requirement (Gustavo):** under the 1er-llamado convenio, the customer can set a **specific payment due date per product** (inscripción, septiembre, seguro escolar, enciclopedias). Not applicable to 2do/3er llamado.

**Recommended approach (pending approval):**
1. `ueipab_payment_due_date` (Date) on `sale.order.line` — editable only on L1/convenio quotes
2. PDF: new "CONVENIO DE PAGO" section — table Concepto | Monto USD | Fecha acordada (turns the Acuerdo into the actual convenio document)
3. Wizard/button "Generar Plan de Pagos" on confirm → one draft invoice per due-date group, `invoice_date_due` = agreed date
...
- [ ] Date limits per concept (e.g. inscripción by ?, septiembre by 30/09?, seguro/enciclopedias by ?)
- [ ] Who sets the dates: Sales team in UI only (recommended phase 1) or also Glenda conversationally?
- [ ] Invoices upfront at convenio signing (recommended — visible in statement, reminders work) vs generated by cron on each date?
