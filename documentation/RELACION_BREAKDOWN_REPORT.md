...
| 2026-04-19 | v1.62.5 | Fix: Prestaciones Soc. Intereses report used historical per-month BCV rates to accumulate interest (summing `interest/month × historical_rate_N`), causing total to differ from email template. Added `exchange_rate` + `exchange_rate_date` fields to wizard (auto-filled from latest BCV, user-editable). Report now uses wizard rate for interest amounts; historical rate kept in "Tasa del Mes" display column only. Rate shown in PDF header. |
| 2026-04-19 | v1.62.6 | Fix (two issues): (1) Relación net was off by ~Bs 4.80 because it summed individually-converted lines (`$545.20 × rate`) instead of using the stored `LIQUID_NET_V2` (`$545.19 × rate`) — a $0.01 float rounding gap already in payslip data. Fixed: net now uses `_convert_currency(net, usd, currency, date_ref, exchange_rate)` directly. (2) Prestaciones Interest last-row accumulated interest was off by ~Bs 108 because `int(service_months=11.13)=11` drops 0.13 fractional months. Fixed: after loop, last row's `accumulated_interest` is set to `total_interest_converted` so running total reconciles to the footer. |
