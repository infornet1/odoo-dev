...
**Status:** IN PROGRESS | **Created:** 2026-06-12 | **Updated:** 2026-06-30 (v0.15.1–0.15.3 — grade-classification fix + clause softening + WA fallback) | **Owner:** Gustavo Perdomo
...
- [x] **Phase 5 — Step 6 contract (QWeb PDF):** ✅ 2026-06-13 → **v0.4.1 2026-06-15** — 2-page "Contrato Servicio Educativo Privado - Orden de Servicio" QWeb PDF bound to `enrollment.journey`. Page 1: institution + representante + Section C (students + insurance merged: name/cédula/grade/póliza) + Section E (ceiling summary: $218,88 × N students × 13 payments = max obligation) + Section F (early payment benefit + **QR code**) + dual signatures. Page 2: full 10-clause T&C (Cláusula 4 Plazo de Pago: "pagarse" — corrected 2026-06-15). Auto-sequence `CSE-2627-XXXX` on first print.
  ...
  - **v0.15.1 — Grade-classification fix (2026-06-30, both envs + prod):** `_is_graduating_grade(grade)` previously tested `g.startswith('5') and 'a' in g` — the bare `'a'` also matched the *'a'* in **"Grado"**, so **"5to. Grado"** (5th grade *primaria*, a **continuing** student) was mis-classified as graduating and silently dropped from `_enrolling_students()` → undersized auto-quotes and hid those students from the S0 page. Fixed to `g.startswith('5') and ('año' in g or 'ano' in g)` so only **"5to. Año"** (bachillerato terminal year) graduates. **ALSO** `_next_grade` rewritten from a naïve leading-digit `+1` to a canonical Venezuelan grade ladder (`_GRADE_LADDER` / `_GRADE_NEXT`): handles cross-level transitions **6to. Grado → 1er. Año** (primaria→bachillerato), **3er Grupo → 1er. Grado** (preescolar→primaria), **5to. Año → ''** (graduating terminal). The old increment produced bogus **"7to. Grado" / "6to. Año"** and ungrammatical **"2er. Año"**.
  - **v0.15.2 — Contract clause softening (2026-06-30, both envs + prod):** the electronic-capture sentence in **Contrato Cl.11** (and **Acuerdo Cl.10** in `ueipab_sales` 1.2.6) made conditional + permissive: *"Cuando la aceptación se efectúe por medios electrónicos, EL PRESTADOR DE SERVICIOS **podrá** registrar y conservar la fecha/hora (UTC), IP, dispositivo, hash SHA-256 …"* — so the clause never over-states data capture for the **v1 in-person / manual acceptance path** (v1 does **not** capture an e-signature; the clause stands as contract text only).
  - **v0.15.3 — WhatsApp fallback in the S0 blast (2026-06-30, both envs + prod):** `action_send_blast_email()` now **auto-falls-back to WhatsApp** when a representante has **no email** (typically a prior hard bounce). New core `_send_wa_s0()` prefers the canonical `ai.agent.whatsapp.service.send_message` (param-based config / credit-guard / anti-spam throttle → works on prod; dev-file fallback when the service is absent); `action_send_wa()` is now a thin wrapper over it. Context test overrides: **`s0_test_email`** (force the email recipient) + **`s0_test_phone`** (force the WA recipient, tags the message **[PRUEBA]**, stamps **no** state) — for safe test sends. The blast's final notification reports the **email / WA split**.
...
**Infra:** the :8019 nginx vhost now forwards `Host $http_host` (preserves the port so post-POST 303 redirects don't 404) and already sets `X-Forwarded-For`/`X-Real-IP`. ✅ **PROD DEPLOYED 2026-06-29:** (a) module scp'd + installed (`ueipab_enrollment_journey` 17.0.0.14.0, `ueipab_sales` 17.0.1.2.5); (b) **no nginx whitelist needed** — the prod `odoo.ueipab.edu.ve` vhost has no route allow-list, the catch-all `location /` already serves `/enrollment-journey`, `/verify-quote`, `/verify-contract` (verified: 404 from Odoo on fake tokens = routed); (c) prod vhost **already** forwards `X-Forwarded-For`/`X-Real-IP` + `Host $http_host` → IP capture works with no change. See ENROLLMENT_PROCESS_PROD_DEPLOYMENT_ASSESSMENT.md §13. ⚠️ B6 counsel sign-off still pending before any parent-facing send.
