...
| 17.0.1.59.0 | Stage 1 email: ARC summary table injected (monthly detail + totals + AR-I%); `AGUINALDOS_2025` excluded from payslip search so months with only aguinaldo payslip fall through to contract simulation; simulation fixes: PARO=0 for regular nómina (applies to Utilidades only), ARI based on `ueipab_salary_v2` only (mirrors `VE_ARI_DED_V2` rule) |
