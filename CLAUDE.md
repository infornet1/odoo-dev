...
| ueipab_payroll_enhancements | 17.0.1.75.0 | **v1.75.0 (2026-07-01, both + prod): invoice-reminder wizard redesigned to a two-step config→preview flow** — onchange population REMOVED (it left the list stale/empty on filter switch = "filters not working" report; server-side `_compute_lines()` was always correct); wizard now opens in `state='config'` (filters only, no list) → footer «Ver lista →» (`action_load_list`, warm-reload = Owl-crash-safe) → `state='preview'` (readonly filter recap + confirmation list + Email/WA send buttons + «← Cambiar filtros»). New param-driven banner `wa_invoice_reminder.wa_notice` (set both envs: Glenda primary WA +58 414-8321989 down, Massiva ticket, sends via backup — clear param to hide). Prod backups `*.bak-20260701`. both — `is_advance_payment` on individual payslips + `has_period_advance` double-pay guard + amber email banner + invoice-reminder wizard (2026-06-23): `all` segment ("Todos con saldo pendiente" — **implies PDVSA override**, no toggle) + `override_pdvsa_rule` toggle for segmented modes + `_sync_eligibility()` authoritative send + **WA button now sends the wizard's EXACT selected list** (ad-hoc payload `wa_invoice_reminder.adhoc_payload` → `wa_invoice_reminder.py --adhoc`, Odoo `mobile` phones; force-dry while `ai_agent.dry_run=True`). ⚠️ populate-on-open via server action REVERTED in v1.74.1 — COLD-mounting the form with rows present re-triggers the Owl `this.fiber.bdom is null` crash; list must fill post-mount via onchange (warm reloads from in-form buttons are fine) |
| ueipab_hr_contract | 17.0.2.0.0 | both |
| ueipab_bounce_log | 17.0.1.4.0 | both |
| ueipab_ai_agent | 17.0.1.61.0 | **v1.61.0 (2026-07-01, both + PROD — DORMANT): WhatsApp provider abstraction for the MassivaMóvil→Kapso migration.** `ai.agent.whatsapp.service` is now a facade dispatching per `ir.config_parameter ai_agent.wa_provider` ('massiva' default \| 'kapso') — zero call-site changes. New `ai.agent.kapso.service` (Meta Cloud API via Kapso proxy: `POST api.kapso.ai/meta/whatsapp/v23.0/{pnid}/messages`, X-API-Key) + webhook `/ai-agent/kapso/webhook` (`type='http'`, HMAC-SHA256 `X-Webhook-Signature`, **fail-closed** when Kapso active w/o secret) → `_handle_kapso_inbound` → same `action_process_reply` pipeline; dedup on new `ai.agent.message.kapso_message_id` Char + `UNIQUE` constraint. Kill switches (`wa_credits_ok`, `dry_run`) preserved; `_check_whatsapp_credits` short-circuits healthy under Kapso. Config `config/kapso_api.json` seeded by post-init hook. **Deployed dormant (provider stays massiva; prod has NO kapso creds yet).** Passed a 13-finding adversarial review (fixes: fail-closed webhook, SSRF media-URL guard, unique-constraint dedup, 502-on-error redelivery, media-type propagation, empty-message guard). ⚠️ Pre-cutover Phase-2: deferred/dry_run message persistence + full SSRF helper. Cutover gated on the dedicated number +58 414-8321989 landing on Kapso + a prod API key. See WA_PROVIDER_MIGRATION_KAPSO.md. + **v1.60.5 (2026-06-30): Glenda taught the 2026-2027 continuity/enrollment process** — new `_INSTITUTIONAL_KNOWLEDGE` section: S0 survey + `/enrollment-journey` link → route to **inscripcion@** (never invent links); **anticipos / facturación fraccionada FAQ** ("varias facturas por un anticipo NO es doble cobro"); grade progression (5° Año gradúa vs 5° Grado continúa); egreso; contingencia. Follow-ups pending: per-conversation enrollment-status injection + `ACTION:ENROLLMENT_LINK`/`CONFIRM_CONTINUITY` + `ENROLL_<token>` Telegram handler. both — **Voice Calls — DEPLOYED TO PROD 2026-06-30** (capability live in DB_UEIPAB; ⚠️ parent-facing calling GATED on caller-ID + consent — kill switch `voice_call.enabled`). Outbound PSTN voice for Glenda via a standalone Twilio↔OpenAI-Realtime gateway (`voice_gateway/`, systemd `glenda-voice` on ueipab2; g711 µ-law passthrough; voice `sage` + `gpt-realtime-2`; Venezuelan-accent prompt). Public ingress = **`voice.ueipab.edu.ve`** (nginx+TLS, tunnel retired); prod↔gateway over VPC `8090`/`8075` (DO fw VPC-only). Smoke-tested prod 2026-06-30 (62s, transcript on DB_UEIPAB record). Odoo side adds `ai.agent.voice.call` model + menu **AI Agent → Operaciones → Llamadas de Voz** (status/duration/transcript/disposition + 📞 button) + **Configuracion → Ajustes de Voz** (`res.config.settings`, `voice_call.*` params) + controller `/ai-agent/voice/callback` + `/ai-agent/voice/tool` + **live function tools** `get_pricing`/`get_balance`. Caller ID temp = US `+15093843032` (Movistar SIM pending; WA-backup `+584248944898` un-verifiable = WABA-locked). See GLENDA_VOICE_CALL_POC.md. + **Conversation loop bug fix (2026-06-24, RAIZA RENDON)**: `_TD_LABEL`/`_TD_VAL` NameError in receipt-email builder rolled back the poll-cron message log → dedup miss → resend loop + empty orphaned convs. v59.7 = `self._TD_LABEL` + receipt block in its own savepoint; v59.6 = reopen recently-resolved conv (`ai_agent.reopen_resolved_window_min`, 30 min) + per-PHONE Telegram-footer guard; v59.8 = orphan hardening (`newly_created_ids` Phase-2 cleanup + `_sweep_empty_conversations()` age-gated). See GLENDA_CONVERSATION_LOOP_BUG.md. + Cashea-reminder FAQ (2026-06-23): Glenda explains why balance reminders persist after a Cashea payment (Cashea remits progressively as the parent pays cuotas → automated reminders until the cycle closes; "su pago está registrado"). ⚠️ **Banco de Venezuela UNAVAILABLE** (2026-06-23): Glenda's MEDIOS DE PAGO knowledge no longer offers BdV (0102); see note below. + ACTION:QUOTE + live pricing ground truth (2026-06-11) |
| ueipab_attendance_report | 17.0.1.6.36 | **v1.6.36 (2026-07-01, both + prod): smart check-out suggestion on the correction form** (`attendance_fix.py` `_suggest_checkout()`) — pre-selects a check-out time from the employee's own history: ≥3 clean records on the same weekday → weekday median, else ≥5 clean any-day → all-day median, else default; "clean" = session ≥3 h (filters auto-fill artifacts), rounded to 5 min + clamped 07:00–21:00 VET; blue hint "Sugerido según tu historial…". + **Contingencia Académica survey CLOSED early** (`partner_ack.py` `_VOTE_DEADLINES` gate moved 2026-07-01→2026-06-30 per CEO, threshold 90 SÍ surpassed at 115; closed page still DISPLAYS the promised "01 de julio de 2026"). — **v1.6.35 (2026-06-30): loyalty-branded ACK** on the `partner.communication.ack` stack for notice_key `loyalty_2026_2027` — additive `partner_ack.py` branches: `_vote_context` "Beneficio de fidelidad aceptado", new `_page_loyalty_success`, receipt **from/CC pagos@** (Carta de Fidelidad campaign — flat $162,39 frozen all year; VIP-only recipient tier = 4 families; see CARTA_FIDELIDAD_2026_2027.md). **v1.6.32–1.6.34 (2026-06-30, both envs + prod):** kiosk **zero-duration toggle server-side guard** (`models/hr_employee_kiosk_guard.py` overrides `hr.employee._attendance_action_change`; a re-toggle within `attendance.kiosk_min_toggle_seconds`=60 is a no-op → prevents `check_in==check_out` rows the v1.6.28 JS in-flight lock missed — real cases DIXIA/LEIDYMAR/MAIRELSY) + **"Notas de auditoría" empty-box view fix** (partner.communication.ack form, hides empty audit textarea). **both prod-deployed 2026-06-29.** v1.6.29–1.6.31 add the **Contingencia Académica SÍ/NO survey** on the `partner.communication.ack` stack (notice_key `contingencia_academica_2026`; `partner_ack.py` notice-key-aware pages + votacion@ confirmations; **v1.6.31 fixes the "Voto Asistido" wizard to static SÍ/NO** — a `fields_get` override on `default_ack_id` does NOT work in the UI, web client strips `default_*`/caches view metadata). Survey LIVE: email 169/169 + WA 9/9; digest cron to CEO+director; threshold 90 of 178. See CONTINGENCIA_ACADEMICA_SURVEY.md. — v1.6.28 (2026-06-29) added the **kiosk double-submit guard** (`static/src/js/kiosk_double_submit_guard.js`: re-entrancy lock + `ui.block()` → fixes `could not serialize access`). hr.leave CC fix (RRHH notified on approve/refuse). See ATTENDANCE_DANGLING_OPEN_RECORDS.md |
