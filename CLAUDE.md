...
- **Leave notification "Ver en Odoo" broken URL (2026-06-01):** Both `scripts/leave_notification.py` (lines 218, 234) and `scripts/hr_leave_attendance_digest.py` (line 267) generate `/odoo/time-off/{id}` — the new-style Odoo 17 path that opens the employee self-service view, not the HR management form. Fix: replace with hash URL `{ODOO_BASE}/web#id={id}&cids=1&menu_id=378&action=520&model=hr.leave&view_type=form` (action=520 = "All Time Off", menu=378 = Time Off/Management — confirmed in prod). Same fix needed in both scripts. No module upgrade required.
