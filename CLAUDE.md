...
| ueipab_enrollment_journey | 17.0.0.12.0 | testing only — onboarding (9-step 3-block timeline 🏛️/💻/📁, hard Block 1 gate, contract escrow step 3, Glenda bubble, QWeb PDF CSE-2627-XXXX + QR seal `/verify-contract/<token>`, **S0 continuity gate**) + **withdrawal/egreso** (`enrollment.withdrawal`, 5-step, auto-create on S0 decline) + **Phase 1b Akdemia student import** (live `GET /api/ext/v1/students` → snapshot, `_line_key` blank-cédula-safe match, diff-preview wizard, cron cache `akdemia.students_json`, params `akdemia.api_key`/`base_url`(staging)/`min_cache_guardians`). **v0.12.0 (2026-06-26): auto-quote on S0 'Sí'** — `_ensure_quote()` (idempotent, sizes to enrolling-student count, `create_ai_quote` at confirmation-day llamado) wired into `/confirm`. **Rollout model:** bulk S0 survey = universal trigger; quote = per-confirmed-family auto on 'Sí'. **`scripts/enrollment_journey_mass_create.py`** (DRY default / `LIVE=1`): mass-create journeys, 3-pass per-student **billing-parent dedup** (`max posted invoices → customer_rank → id` reproduces Customers-tab "one valid VAT"). **PROD deploy kit ready (not deployed): `scripts/deploy_enrollment_journey_prod.sh` + `prod_post_deploy_enrollment_journey.py`** — see ENROLLMENT_JOURNEY_WIZARD.md + ENROLLMENT_PROCESS_PROD_DEPLOYMENT_ASSESSMENT.md |
...
**Enrollment:** [Enrollment Journey Wizard](documentation/ENROLLMENT_JOURNEY_WIZARD.md) · [Withdrawal Plan](documentation/ENROLLMENT_WITHDRAWAL_PLAN.md) · [Prod Deployment Assessment](documentation/ENROLLMENT_PROCESS_PROD_DEPLOYMENT_ASSESSMENT.md) — `ueipab_enrollment_journey` **v0.12.0 testing-only**; onboarding (9-step 3-block timeline + S0 gate + QWeb CSE-2627 PDF + QR) + withdrawal/egreso + Phase 1b Akdemia import + **auto-quote on S0 'Sí'** + **mass-create script w/ billing-parent dedup**; **prod deploy kit built, NOT yet deployed** (assessment verdict: conditionally ready — no hard blockers, clear B1 report host + akdemia.api_key + pilot).
