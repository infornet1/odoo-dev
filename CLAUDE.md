...
| ueipab_sales | 17.0.1.2.1 | both — quotation engine + convenio payment plan + `get_pricing_ground_truth()` + T&C annex page 2 on Acuerdo PDF (2026-06-12) (see UEIPAB_SALES_QUOTATION_PLAN.md) |
| ueipab_enrollment_journey | 17.0.0.2.0 | testing only — 6-step customer timeline page + hybrid staff clearance + Glenda bubble + 2-page "Contrato Servicio Educativo Privado" QWeb PDF (sequence CSE-2627-XXXX) — see ENROLLMENT_JOURNEY_WIZARD.md |
...
- **`LIQUID_UTILIDADES_V2` rate inconsistency (2026-06-05):** Rule uses 15 days/year (LOTTT minimum) but UEIPAB's aguinaldo policy pays 60 days/year (2× monthly salary). Under LOTTT Art. 131, utilidades and aguinaldos are the same concept. Additionally, the formula uses full `service_months` which overlaps with the Dec aguinaldo already paid for the prior fiscal year — proportional utilidades in liquidation should cover only the current fiscal year up to termination (e.g. Jan–Jun). Decision needed: (1) align rate to company policy (60 days) or keep LOTTT minimum (15 days); (2) fix period to current fiscal year only. See CHANGELOG 2026-06-05 for detailed numbers.
