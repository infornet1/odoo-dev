...
| ueipab_attendance_report | 17.0.1.6.28 | **both 1.6.28 (prod deployed 2026-06-29** — was lagging at 1.6.25; this jump also brought the 1.6.27 hr.leave CC fix to prod). v1.6.28 (2026-06-29) adds the **kiosk double-submit guard** (`static/src/js/kiosk_double_submit_guard.js` patches the public kiosk manual path: re-entrancy lock + `ui.block()` → stops concurrent `manual_selection` RPCs → fixes `could not serialize access` log errors; verified live in prod bundle). hr.leave CC fix (RRHH notified immediately on approve/refuse). See ATTENDANCE_DANGLING_OPEN_RECORDS.md |
